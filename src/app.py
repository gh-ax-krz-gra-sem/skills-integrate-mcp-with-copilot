...
def login(username: str, password: str):
    ...
    for teacher in teachers:
        if teacher['username'] == username and teacher['password'] == password:
            # Create a simple session token (username-based for simplicity)
            session_token = f"session_{username}"
            active_sessions[session_token] = username
            return {"success": True, "token": session_token, "username": username}
...
def verify_teacher_auth(authorization: Optional[str] = Header(None)):
    ...
    if not authorization or authorization not in active_sessions:
        raise HTTPException(status_code=401, detail="Authentication required. Only teachers can perform this action.")
